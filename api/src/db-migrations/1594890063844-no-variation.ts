...
export class noVariation1594890063844 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`alter table product add column without_variation boolean not null default true;`);
    await queryRunner.query(`alter table product_variation add column no_variation boolean not null default false;`);
    await queryRunner.query(`update product set without_variation = true where variations_size = 0;`);
    await queryRunner.query(`update product set without_variation = false where variations_size > 0;`);
